...
def main() -> int:
    ...
    for idx, case_raw in enumerate(cases, start=1):
        ...
        onset = (crack_delta >= min_crack_delta) or (crack_mean_delta >= min_crack_mean_delta)
        passed = error is None and finite_ok and warning_ok and mech_ok and crack_cg_ok and nonfinite_ok and onset
    ...
    checks_passed = all(
        (
            r.get("error") is None
            and bool(r.get("finite_ok", False))
            and bool(r.get("warning_ok", False))
            and bool(r.get("mech_ok", False))
            and bool(r.get("crack_cg_ok", False))
            and bool(r.get("nonfinite_ok", False))
        )
        for r in case_rows
    )
    ...
    if not checks_passed:
        failed = [str(r.get("name", "")) for r in case_rows if not bool(r.get("passed", False))]
    ...
    with csv_path.open("w", encoding="utf-8") as fh:
        fh.write(
            "name,passed,onset,cycles_completed,crack_delta,crack_mean_delta,"
            "crack_length_final,crack_mean_final,runtime_warning_count,"
            "finite_ok,warning_ok,mech_ok,crack_cg_ok,nonfinite_ok\n"
        )
        for r in case_rows:
            fh.write(
                ...
                f"{int(bool(r['mech_ok']))},{int(bool(r['crack_cg_ok']))},{int(bool(r['nonfinite_ok']))}\n"
            )
