"""
Lightweight regression checks for boundary handling and crack driving.
...
def _test_pure_compression(split_mode: str = "spectral") -> float:
    ...
    for _ in range(5):
        solver.step((-0.005, 0.0, 0.0))
    phi1 = solver.state["crack"]
    return float(np.max(phi1 - phi0))


def _test_patch() -> Tuple[float, float]:
    grid = GridSpec(shape=(12, 12, 12), spacing=(1.0, 1.0, 1.0), periodic=(True, True, True))
    orientation = _make_orientation(grid)
    copper = CopperParameters()
...
def run(thr: Thresholds) -> tuple[Report, Dict[str, Dict[str, float] | str]]:
    ...
    t0 = perf_counter()
    t = perf_counter()
    comp_growth = _test_pure_compression()
    timings["compression_s"] = perf_counter() - t
    t = perf_counter()
    div_norm, ratio = _test_patch()
    timings["patch_s"] = perf_counter() - t
    t = perf_counter()
    phi0, phi1 = _test_mode_I()
